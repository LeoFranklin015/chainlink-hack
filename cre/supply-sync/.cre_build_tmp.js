...
class IERC20 {
  ...
  allowance(runtime3, owner, spender) {
  ...
  writeReportFromApprove(runtime3, spender, amount, gasConfig) {
    const callData = encodeFunctionData({
      abi: IERC20ABI,
  ...
  writeReportFromTransfer(runtime3, recipient, amount, gasConfig) {
    const callData = encodeFunctionData({
      abi: IERC20ABI,
  ...
  writeReportFromTransferFrom(runtime3, sender, recipient, amount, gasConfig) {
    const callData = encodeFunctionData({
      abi: IERC20ABI,
  ...
  logTriggerApproval(filters) {
    ...
    if (!filters || filters.length === 0) {
      ...
      topics = encoded.map((t) => ({ values: [hexToBase642(t)] }));
      ...
      topics = allEncoded[0].map((_, i2) => ({
        values: [...new Set(allEncoded.map((row) => hexToBase642(row[i2])))]
      }));
    }
    const baseTrigger = this.client.logTrigger({
      addresses: [hexToBase642(this.address)],
      topics
    });
    const contract = this;
    return {
      ...
      adapt: (rawOutput) => contract.decodeApproval(rawOutput)
    };
  }
  decodeApproval(log) {
    const decoded = decodeEventLog({
      abi: IERC20ABI,
      data: bytesToHex4(log.data),
      topics: log.topics.map((t) => bytesToHex4(t))
    });
    const { data: _, ...rest } = log;
    return { ...rest, data: decoded.args };
  }
  logTriggerTransfer(filters) {
    let topics;
    if (!filters || filters.length === 0) {
      const encoded = encodeEventTopics({
        abi: IERC20ABI,
    ...
    return {
      ...
      outputSchema: () => baseTrigger.outputSchema(),
      configAsAny: () => baseTrigger.configAsAny(),
      adapt: (rawOutput) => contract.decodeTransfer(rawOutput)
    };
  }
  decodeTransfer(log) {
    const decoded = decodeEventLog({
...
class XStocksExchange {
  ...
  exceedsHoldingLimit(runtime3, holder) {
    const callData = encodeFunctionData({
      abi: XStocksExchangeABI,
      functionName: "exceedsHoldingLimit",
      args: [holder]
    });
    const result = this.client.callContract(runtime3, {
      call: encodeCallMsg({ from: zeroAddress, to: this.address, data: callData }),
  ...
  flaggedHolders(runtime3, holder) {
    const callData = encodeFunctionData({
      abi: XStocksExchangeABI,
      functionName: "flaggedHolders",
      args: [holder]
    });
    const result = this.client.callContract(runtime3, {
      call: encodeCallMsg({ from: zeroAddress, to: this.address, data: callData }),
  ...
  crossChainSupplyValue(runtime3) {
    const callData = encodeFunctionData({
      abi: XStocksExchangeABI,
      functionName: "crossChainSupply"
    });
    const result = this.client.callContract(runtime3, {
      call: encodeCallMsg({ from: zeroAddress, to: this.address, data: callData }),
      blockNumber: LATEST_BLOCK_NUMBER2
  ...
  writeReportFromFlagHolder(runtime3, holder, gasConfig) {
    const callData = encodeFunctionData({
      abi: XStocksExchangeABI,
  ...
  writeReportFromUnflagHolder(runtime3, holder, gasConfig) {
    const callData = encodeFunctionData({
      abi: XStocksExchangeABI,
      functionName: "unflagHolder",
      args: [holder]
    });
    const reportResponse = runtime3.report(prepareReportRequest(callData)).result();
    return this.client.writeReport(runtime3, {
  ...
  writeReportFromVerifyOffchain(runtime3, user, nullifierHash, gasConfig) {
    const callData = encodeFunctionData({
      abi: XStocksExchangeABI,
  ...
  logTriggerUserVerified(filters) {
    let topics;
    if (!filters || filters.length === 0) {
      const encoded = encodeEventTopics({
        abi: XStocksExchangeABI,
    ...
    return {
      ...
      outputSchema: () => baseTrigger.outputSchema(),
      configAsAny: () => baseTrigger.configAsAny(),
      adapt: (rawOutput) => contract.decodeUserVerified(rawOutput)
    };
  }
  decodeUserVerified(log) {
    const decoded = decodeEventLog({
      abi: XStocksExchangeABI,
      data: bytesToHex4(log.data),
      topics: log.topics.map((t) => bytesToHex4(t))
...
var onCronTrigger = (runtime3, payload) => {
  ...
  for (const chain of chains) {
    ...
    const evmClient = new cre.capabilities.EVMClient(network495.chainSelector.selector);
    const token = new IERC20(evmClient, chain.tokenAddress);
    let supply;
    try {
      supply = token.totalSupply(runtime3);
    } catch (e) {
      runtime3.log(`Failed to read totalSupply on ${chain.chainSelectorName}, defaulting to 0`);
      supply = BigInt(0);
    }
    supplies.push(supply);
    runtime3.log(`${chain.chainSelectorName} totalSupply: ${supply.toString()}`);
  }
  let totalGlobalSupply = BigInt(0);
  for (const supply of supplies) {
    totalGlobalSupply += supply;
  }
  runtime3.log(`Total global supply: ${totalGlobalSupply.toString()}`);
  for (let i2 = 0;i2 < chains.length; i2++) {
    const chain = chains[i2];
    const localSupply = supplies[i2];
    ...
    const evmClient = new cre.capabilities.EVMClient(network495.chainSelector.selector);
    const exchange = new XStocksExchange(evmClient, chain.exchangeAddress);
    runtime3.log(`Writing crossChainSupply=${crossChainSupply.toString()} to ${chain.chainSelectorName}`);
    const resp = exchange.writeReportFromSetCrossChainSupply(runtime3, crossChainSupply, { gasLimit: chain.gasLimit });
    if (resp.txStatus !== TxStatus.SUCCESS) {
      throw new Error(`Failed to set crossChainSupply on ${chain.chainSelectorName}: ${resp.errorMessage || resp.txStatus}`);
    }
    const txHash = resp.txHash || new Uint8Array(32);
    runtime3.log(`crossChainSupply updated on ${chain.chainSelectorName}: ${bytesToHex(txHash)}`);
  }
