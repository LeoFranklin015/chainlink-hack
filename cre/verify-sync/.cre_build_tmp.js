...
var configSchema = exports_external.object({
  ...
  targetChains: exports_external.array(chainSchema)
});
var onUserVerifiedTrigger = (runtime3, payload) => {
  const { targetChains } = runtime3.config;
  const { user, nullifierHash } = payload.data;
  runtime3.log(`UserVerified event: user=${user}, nullifierHash=${nullifierHash.toString()}`);
  const results = [];
  for (const chain of targetChains) {
    ...
    if (!network495) {
      runtime3.log(`Network not found: ${chain.chainSelectorName}, skipping`);
      continue;
    }
    ...
    runtime3.log(`Syncing verification to ${chain.chainSelectorName}`);
    const resp = exchange.writeReportFromVerifyOffchain(runtime3, user, nullifierHash, {
    ...
    runtime3.log(`Verified on ${chain.chainSelectorName}, tx: ${bytesToHex(txHash)}`);
    results.push(`synced:${chain.chainSelectorName}`);
  }
  if (results.length === 0)
    results.push("no_targets");
  return results.join(",");
};
function initWorkflow(config) {
  ...
  const evmClient = new cre.capabilities.EVMClient(network495.chainSelector.selector);
  const exchange = new XStocksExchange(evmClient, primaryChain.exchangeAddress);
  return [
    cre.handler(exchange.logTriggerUserVerified(), onUserVerifiedTrigger)
  ];
}
async function main() {
