...
class XStocksPriceReceiver {
  ...
  writeReportFromUpdatePrice(runtime3, price, gasConfig) {
    const callData = encodeFunctionData({
      abi: XStocksPriceReceiverABI,
      functionName: "updatePriceFromReport",
      args: [price]
    });
  ...
  logTriggerPriceUpdated(filters) {
    let topics;
    ...
    return {
      ...
      adapt: (rawOutput) => contract.decodePriceUpdated(rawOutput)
    };
  }
  decodePriceUpdated(log) {
...
var chainSchema = exports_external.object({
  ...
  priceReceiverAddress: exports_external.string(),
  gasLimit: exports_external.string()
});
var configSchema = exports_external.object({
  schedule: exports_external.string(),
  finnhubApiKey: exports_external.string(),
...
var writePriceToChain = (runtime3, chainConfig, priceReceiverAddress, symbol3, priceScaled) => {
  ...
  const receiver = new XStocksPriceReceiver(evmClient, priceReceiverAddress);
  runtime3.log(`Writing ${symbol3} price ${priceScaled.toString()} to ${priceReceiverAddress} on ${chainConfig.chainSelectorName}`);
  const resp = receiver.writeReportFromUpdatePrice(runtime3, priceScaled, { gasLimit: chainConfig.gasLimit });
  if (resp.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to write ${symbol3} price on ${chainConfig.chainSelectorName}: ${resp.errorMessage || resp.txStatus}`);
  }
  const txHash = resp.txHash || new Uint8Array(32);
  runtime3.log(`${symbol3} price update on ${chainConfig.chainSelectorName} succeeded: ${bytesToHex(txHash)}`);
};
var onCronTrigger = (runtime3, payload) => {
  if (!payload.scheduledExecutionTime) {
    throw new Error("Scheduled execution time is required");
  }
  const { symbol: symbol3, chains, finnhubApiKey } = runtime3.config;
  const httpClient = new cre.capabilities.HTTPClient;
  runtime3.log(`Fetching ${symbol3} price from Finnhub`);
  const priceScaled = httpClient.sendRequest(runtime3, fetchPrice, consensusMedianAggregation())({ symbol: symbol3, finnhubApiKey }).result();
  runtime3.log(`${symbol3} consensus price (${PRICE_DECIMALS} decimals): ${priceScaled}`);
  const priceBigInt = BigInt(priceScaled);
  for (const chainConfig of chains) {
    writePriceToChain(runtime3, chainConfig, chainConfig.priceReceiverAddress, symbol3, priceBigInt);
  }
  return `${symbol3}=${priceScaled}`;
};
