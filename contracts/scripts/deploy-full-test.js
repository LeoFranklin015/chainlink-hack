...
async function main() {
  ...
  let tx = await priceReceiver.updatePrice(100000000); // $1.00 in 8 decimals
  await tx.wait();
  ...
  // 6. ExchangeOnlySanctionsList (blocks direct P2P transfers)
  ...
  console.log("Token roles set (sanctions, pauser, minter, burner)");

  // 8. Set transfer lock on exchange
  tx = await exchange.setTransferLock(sanctionsList.address); await tx.wait();
  console.log("Transfer lock set on exchange");

  // 9. Verify deployer for testing (bypass World ID)
  tx = await exchange.setVerifiedUser(deployer.address, true); await tx.wait();
  console.log("Deployer verified for testing");

  // 10. Set max holding to 100% for bootstrap, then lower after first buy
  tx = await exchange.setMaxHoldingBps(10000); await tx.wait();
  console.log("Max holding set to 100% for bootstrap");
  ...
  if (usdcBal.gte(1000000)) {
    tx = await usdc.approve(exchange.address, 1000000);
    await tx.wait();
    tx = await exchange.buy(1000000, { gasLimit: 500000 }); // 1 USDC
    await tx.wait();
    const tokenBal = await token.balanceOf(deployer.address);
    console.log("Buy succeeded! Token balance:", hre.ethers.utils.formatEther(tokenBal));

    // Lower holding limit back to 5%
  ...
  const addresses = {
    ...
    maxHoldingBps: 500,
    testPrice: "$1.00",
    transferRestriction: "exchange-only (ExchangeOnlySanctionsList)",
    deployedAt: new Date().toISOString(),
  };
  const addressesPath = path.resolve(__dirname, "../deployed-addresses-test.json");
